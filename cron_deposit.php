        ...
        if ($address['total_received'] > 0) {
            ...
            if (Users::checkExistByInvestmentAddress($address['address'])) {
                ...
                if (!is_null($telegram_id)) {
                    ...
                    $totalConfirmedForThisAddressInBTC = TRX::TRX(Wallet::getConfirmedReceivedByAddress($address['address']));
                    ...
                    if ($confirmedNewDepositInBtc > 0) {
                        ...
                        if ($balanceConfirmed >= InvestmentPlan::getValueByName('minimum_invest')) {
                            ...
                            $data = [
                                ...
                                'text' => 'Your deposit of <b>' . TRX::Format($confirmedNewDepositInBtc) . '</b> is now accepted and your balance of ' . TRX::Format($balanceConfirmed) . ' is invested. You will recover this amount with interest in your balance at the end of your contract.'
                            ];
                            ...
                            $data = [
                                'parse_mode' => 'HTML',
                                'chat_id' => $user->getTelegramId(),
                                'text' => 'Your deposit of <b>' . TRX::Format($confirmedNewDepositInBtc) . '</b> is now accepted but is not higher to invest. You have now an amount of ' . TRX::Format($balanceConfirmed) . ' BTC. The minimum invest is ' . TRX::Format(InvestmentPlan::getValueByName('minimum_invest')) . ' BTC.'
                            ];
                            file_get_contents("https://api.telegram.org/bot$apiToken/sendMessage?" . http_build_query($data));
                        }
                    }
